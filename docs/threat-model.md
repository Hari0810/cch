...
| Provider accepts the connection and hangs | **No timeout on the scorer's `fetch`.** The request hangs for as long as the runtime allows. `AbortSignal.timeout` is used for the Modal call in [`baseline.ts`](../src/lib/baseline.ts) but not for the model call. | **Known, unhandled.** One-line fix; not made under freeze because it touches the scoring path and would need `verify-demo.ts` re-run. |
